...
### H7. `app.js` — вращение фигур: несогласованные матрицы и сломанный I-piece rot 3 🟠 [✅ исправлено]

- **Файл:** `app.js`, константа `SHAPES` + метод `GameEngine.rotate()`.
- **Проблемы:**
  1. S, Z, J, L имели матрицы 3×2 или 3×1 в нечётных вращениях вместо 3×3 — ломалось `isValid()` и рендеринг
  2. I piece rotation 3 — блок в колонке 3 вместо колонки 2 (нецентрировано)
  3. Wall kicks — только 5 позиций, недостаточно для диагональных/дальних случаев
- **Исправление:**
  - Все фигуры приведены к единым размерам: I=4×4, O=2×2, T/S/Z/J/L=3×3 для всех 4 вращений
  - I rot 3: блок теперь в колонке 2 (центрирован)
  - Wall kicks расширены до 11 позиций (включая диагональные: `{-1,-1},{1,-1},{-1,1},{1,1}` и дальние: `{-2,0},{2,0}`)
- **Верификация:** Все 4 вращения каждой из 7 фигур являются истинными 90°-поворотами друг друга
